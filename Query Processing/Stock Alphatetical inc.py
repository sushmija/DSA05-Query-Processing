...
# Specify the start and end dates for the line plot
start_date = '2020-04-01'
end_date = '2020-04-20'

filtered_data = df[(df['Date'] >= start_date) & (df['Date'] <= end_date)]

plt.plot(filtered_data['Date'], filtered_data['Close'], marker='o')
plt.xlabel('Date')
plt.ylabel('Close Price')
plt.title(f'Historical Stock Prices of Alphabet Inc. ({start_date} to {end_date})')
