...
sales_data['Sale_amt'] = sales_data['Sale_amt'].replace(',', '', regex=True).astype(float)

pivot_table_region_manager_salesman = pd.pivot_table(sales_data, values='Sale_amt', index=['Region', 'Manager', 'SalesMan'], aggfunc='sum')

print("Total Sale Amount:")
print(pivot_table_region_manager_salesman)
